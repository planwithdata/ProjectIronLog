...
Released into the public domain under [the Unlicense](http://unlicense.org/). No attribution is required; it is recorded here so the provenance stays answerable.

Images were downscaled to 440px wide and re-encoded as WebP (quality 76) so they can be committed and served offline. Two frames per exercise: start and end position.
