/**
 * tools/test-migration.mjs — The v1 -> v2 migration, proven lossless.
 *
...
test('v1 -> v2 preserves every logged set exactly', async () => {
  ...
  for (const [index, expected] of beforeSets.entries()) {
    ...
    assert.equal(actual.exercise, expected.exercise, 'sets stay on their exercise');
...
test('v1 -> v2 preserves session and entry metadata', async () => {
  ...
  for (const [index, original] of before.sessions.entries()) {
    ...
    for (const [entryIndex, entry] of original.entries.entries()) {
      ...
      for (const key of ['exerciseId', 'targetWeightKg', 'targetReps', 'plannedAction',
                          'planReason', 'notes']) {
...
test('the schema version is bumped and recorded', async () => {
  await seedV1(v1Fixture());
  const db = await boot();
  assert.equal(db.read('meta').schemaVersion, 2);
...
test('booting an already-current database runs no migration', async () => {
  ...
  assert.equal(second.read('meta').schemaVersion, 2);
});
...
test('a v1 backup imported into a v2 database is migrated on the way in', async () => {
  await storage.clear();
  await storage.remove('__premigration');
  const db = await boot();
...
test('a v2 export round-trips without loss', async () => {
  await seedV1(v1Fixture());
  const db = await boot();
